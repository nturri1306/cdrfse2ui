...
import it.unidoc.cdr.core.ui.layout.size.Wide;
...
import it.unidoc.cdr.core.ui.util.css.WhiteSpace;
...
public class ListItem extends FlexBoxLayout {
    ...
    /* Allows you  create an automatic list */
    public ListItem(List<String> items) {
        addClassName(CLASS_NAME);
        ...
        this.primary = new Label(items.get(0));

        content = new FlexBoxLayout(this.primary);

        for (int i = 1; i < items.size(); i++)
           content.add(UIUtils.createLabel(FontSize.S, TextColor.SECONDARY, items.get(i)));
