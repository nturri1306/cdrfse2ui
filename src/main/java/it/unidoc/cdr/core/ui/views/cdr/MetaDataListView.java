...
@Route(value = "metadataview", layout = MainLayout.class)
public class MetaDataListView extends CdrServiceView<MetadataFse2> {
    public static final String NAME = "metadataview";

    String collectionName = "cdr_fse2_metadata";
    ...
    protected void addColumns(Grid<MetadataFse2> grid) {
        addColumns(grid, MetadataFse2.class, this.collectionName);
    }
    ...
    public void refresh() {
        ...
        try {
            if (isNotNullOrEmpty(this.filterHolder.getFilter("statusCode"))) {
                ...
                this.grid.setItems(this.dbRepository.getMetadataItems(this.collectionName, pair, code));
            } else if (isByError()) {
                this.grid.setItems(this.dbRepository.getMetadataItems(this.collectionName, pair, -1));
            } else {
                this.grid.setItems(this.dbRepository.getMetadataItems(this.collectionName, pair, 0));
            }
    ...
    protected String getDetailsHeader(MetadataFse2 data) {
        return null;
    }
    ...
    protected FormLayout makeDetails(MetadataFse2 data) {
    ...
    protected void selected(Set<MetadataFse2> items) {}

    protected ExporterWrapper<MetadataFse2> makeExporterWrapper() {
